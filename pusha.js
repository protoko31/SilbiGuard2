...
client1.on("guildBanAdd", async function(guild, user) {
  ...
  guild.members.ban(entry.executor.id, {reason: " Recep Efsanesi System | İzinsiz kullanıcı yasaklama"}).catch(e => { })	
...
client1.on("guildMemberRemove", async kickhammer => {
  ...
  kickhammer.guild.members.ban(yetkili.id, {reason: " Recep Efsanesi System | izinsiz kullanıcı Kickleme!"}).catch(e => { })	
...
const entry = await member.guild
  ...
  member.guild.members.ban(entry.executor.id, {reason: " Recep Efsanesi System | İzinsiz Bot Ekleme!"}).catch(e => { })	
  member.guild.members.ban(member.id, {reason: " Recep Efsanesi System | Bot Koruma Sistemi!"}).catch(e => { })	
...
client1.on("guildUpdate", async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, { reason: ` Recep Efsanesi System | Sunucuyu izinsiz güncellemek.` }).catch(e => { })	
...
client2.on("roleDelete", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: ` Recep Efsanesi System | İzinsiz rol silme!` }).catch(e => { })	
...
client2.on("roleCreate", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: ` Recep Efsanesi System | İzinsiz rol oluşturma!` }).catch(e => { })	
  role.delete({ reason: " Recep Efsanesi System | Rol Koruma Sistemi" }).catch(e => { })	
...
client3.on("channelDelete", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `Pusha System | İzinsiz Kanal Silme!` }).catch(e => { })	
  await channel.clone({ reason: "Pusha System | Kanal Korum Sistemi!" }).then(async kanal => {
...
client3.on("channelCreate", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: ` Recep Efsanesi System | İzinsiz Kanal Oluşturma!` }).catch(e => { })
  channel.delete({reason: " Recep Efsanesi System | Kanal Koruma Sistemi!"}).catch(e => { })
...
client3.on("channelUpdate", async (oldChannel, newChannel) => {
  ...
  newChannel.guild.members.ban(member.id, { reason: ` Recep Efsanesi System | İzinsiz Kanal Güncellemek!` }).catch(e => { })
...
client3.on("webhookUpdate", async (channel) => {
  ...
  await webhooks.map(x => x.delete({reason: " Recep Efsanesi System | Webhook Silindi!"})).catch(err => { });
  channel.guild.members.ban(entry.executor.id, {reason: " Recep Efsanesi System | İzinsiz Webhook Açmak!"}).catch(err => { });
